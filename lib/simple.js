...
/*exports.smsg = (conn, m, hasParent) => {
     ...
     */
     m.reply = async (text, chatId, options) => {
    	let pp = await conn.profilePictureUrl(m.sender, 'image').catch(_ => 'https://telegra.ph/file/215e3f800df177c28a23e.png')
        let { data } = await conn.getFile(await(await require('node-fetch')(pp)).buffer())
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { showAdAttribution: true, title: 'Saxia Bot', body: 'the journey of Saxia', sourceUrl: 'https://Wa.me/6285641142178', thumbnail: data }}, options })
    }
    m.name = m.pushName || conn.getName(m.sender)
